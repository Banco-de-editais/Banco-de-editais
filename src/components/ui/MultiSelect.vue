<template>
  <div ref="root" class="relative">
    ...
    <div v-if="open" class="absolute z-30 mt-2 max-h-64 w-full min-w-56 overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
      <label v-for="option in options" :key="option.id" class="flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800">
      ...
      <p v-if="!options.length" class="px-3 py-4 text-center text-sm text-slate-500">Nenhuma opção disponível.</p>
    </div>
  </div>
</template>
